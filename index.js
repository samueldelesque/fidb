...
var savePath = function(time,format){
	var m = time.getMonth()
	var d = time.getDate()
	var addLeadingZero = function(n){return (n < 10)?"0"+n:n}
	return time.getFullYear()+"-"+addLeadingZero(m)+"-"+addLeadingZero(d)+"-"+addLeadingZero(time.getHours())+"-"+addLeadingZero(time.getMinutes())+"."+format
}
		...
		if (!error && response.statusCode == 200) {
			...
			Csv.fromString(body, {headers: [
				...
				sp500.insert(data)
					...
					console.log(path,"was processed");
			});
